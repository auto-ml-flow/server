...
def index_view(request: HttpRequest) -> HttpResponse:
    ...
    unique_systems_count = (
        SystemModel.objects.values("cpu_name", "gpu_name", "ram").distinct().count()
    )

    # Most popular system
    most_popular_system = SystemModel.objects.annotate(count=Count("id")).order_by("-count").first()

    # Longest run
    longest_run = RunModel.objects.order_by("-duration").first()

    # Average CPU utilization
    avg_cpu_utilization = CPUStatsModel.objects.aggregate(avg_utilization=Avg("utilization"))[
        "avg_utilization"
    ]

    # System with highest memory usage
    highest_memory_usage_system = (
        MemoryStatsModel.objects.order_by("-usage_megabytes").select_related("system").first()
    )
